...
class Aggregator:
    ...
    def _mix_neighbor_vectors(self, neighbor_vectors, neighbor_relations, user_embeddings):
        ...
        if not avg:
            ...
            # input_tensor = in tf.reduce mean
...
class SumAggregator(Aggregator):
    def __init__(self, batch_size, dim, dropout=0., act=tf.nn.relu, name=None):
        ...
        with tf.compat.v1.variable_scope(self.name):
            ...
            self.weights = tf.compat.v1.get_variable(shape=[self.dim, self.dim], initializer=tf.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"), name='weights')
            # self.bias = tf.Variable(tf.zeros(shape=[self.dim]), name='bias')
...
class ConcatAggregator(Aggregator):
    def __init__(self, batch_size, dim, dropout=0., act=tf.nn.relu, name=None):
        ...
        with tf.compat.v1.variable_scope(self.name):
            # self.weights = tf.Variable(tf.keras.initializers.GlorotUniform()(shape = [self.dim, self.dim]), name = 'weights')
            self.weights = tf.compat.v1.get_variable(shape=[self.dim * 2, self.dim], initializer=tf.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"), name='weights')
            # self.bias = tf.Variable(tf.zeros(shape=[self.dim]), name='bias')
            self.bias = tf.compat.v1.get_variable(shape=[self.dim], initializer=tf.compat.v1.zeros_initializer(), name='bias')

    def _call(self, self_vectors, neighbor_vectors, neighbor_relations, user_embeddings):
        # [batch_size, -1, dim]
        neighbor_agg = self._mix_neighbor_vectors(neighbor_vectors, neighbor_relations, user_embeddings)
        # [batch_size, -1, dim * 2]
        output = tf.concat([self_vectors, neighbor_agg], axis = -1)
        # [-1, dim * 2]
...
class NeighborAggregator(Aggregator):
    ...
    def _call(self, self_vectors, neighbor_vectors, neighbor_relations, user_embeddings):
        ...
        neighbor_agg = self._mix_neighbor_vectors(neighbor_vectors, neighbor_relations, user_embeddings)
        # [-1, dim]
        output = tf.reshape(neighbor_agg, [-1, self.dim])
        output = tf.nn.dropout(output, rate=self.dropout)
        output = tf.matmul(output, self.weights) + self.bias
        # [batch_size, -1, dim]
        output = tf.reshape(output, [self.batch_size, -1, self.dim])
